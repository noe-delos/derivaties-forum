-- Run this migration to add the token system to your existing database
-- This should be run AFTER the main migrations.sql has been applied

BEGIN;

-- User purchased content table (for token system)
CREATE TABLE IF NOT EXISTS public.user_purchased_content (
...
CREATE INDEX IF NOT EXISTS idx_user_purchased_content_user_id ON public.user_purchased_content(user_id);
CREATE INDEX IF NOT EXISTS idx_user_purchased_content_post_id ON public.user_purchased_content(post_id);
CREATE INDEX IF NOT EXISTS idx_user_purchased_content_type ON public.user_purchased_content(content_type);
...
-- Add security definer to new functions
...
-- Give existing users 20 tokens if they don't have any
UPDATE public.users SET tokens = 20 WHERE tokens = 0;

COMMIT;

-- Display success message
SELECT 'Token system migration completed successfully!' as message;